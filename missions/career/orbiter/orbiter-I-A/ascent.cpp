...
int main(int argc, char const *argv[])
{
    ...
    /* No nodes available: burn prograde at apoapsis instead. */
    vessel.auto_pilot().set_reference_frame(orbit_reference_frame);
    vessel.auto_pilot().set_target_direction(prograde_direction.to_tuple());

    apoapsis_reached_event.acquire();
    apoapsis_reached_event.wait();
    apoapsis_reached_event.release();

    vessel.control().set_throttle(1.0);

    circularisation_event.acquire();
    circularisation_event.wait();
    circularisation_event.release();
